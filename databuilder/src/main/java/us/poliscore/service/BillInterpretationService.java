 ...
 * ===
 * At one point we experimented with generating different reports for different audiences, primarily an 'expert report' (AI was prompted to write at the 'PHD' level)
 * and a 'casual report' (AI was prompted to write at the High School or below level). This strategy was ultimately abandoned due to the fact that it failed to produce
 * the summary we ultimately wanted most: a detailed report for your average person, made as accessible as possible. The 'PHD' report was decided as undesireable for 
 * two unexpected reasons: 1. The reports it produced tended to be a little pedantic, confusing, and ultimately verbose, producing information that if you really needed
 * you would probably be capable of generating yourself anyway (and you also likely wouldn't trust AI). The 'casual' report tended to be a little condescending, overly
 * simplistic and not as useful as we would like. 2. It influenced the probability space of the output in potentially undesirable ways. If we lean the prompt more towards
 * 'PHD' output, then it tends the skew the impact analysis towards 'PHD' opinions, which you might think would be desireable, however I found that it actually just introduces
 * a bias which doesn't appear desireable. Adding more demands in the 'casual' report also skewed the impact analysis in interesting ways (more towards 'common folk' opinions).
 * Ultimately, this approach was abandoned because I utlimately don't have resources to dig through all this complexity. It might be possible to do something like this in the
 * future but for the moment I just need something simple that works for the majority of audiences.
 */
@ApplicationScoped
public class BillInterpretationService {
	...
	public static final String statsPromptTemplate = """
			...
			Long Report:
			Your first paragraph will explain the high level goals of the bill, give a high level summary of how it attempts to achieve those goals, and then conclude by giving your opinion on the impact the bill will have on society, if enacted. After your high-level summary, you will then go on to explain, in depth, how you came to these conclusions. You may conclude by identifying winners and losers of the bill, and identifying industry stakeholders, if relevant. Your audience here is general public layman voters, so if you think they won't understand an acronym or a complex topic, please explain it. Should be between one and five paragraphs long, depending on the complexity of the bill and the topics it covers. Please bold a word at the beginning of the sentence which signifies what the paragraph is about. If the bill touches on controversial topics such as trans issues or guns rights, please include the advocating logic by proponents and also the advocating logic of the opposition, otherwise do not include this logic. Where relevant, cite scientific studies or the opinions of authoritative knowledge sources to provide more context. Do not include any additional formatting other than markdown links or a singular bold word which helps to easily identify what the paragraph is about. Do not start each paragraph with a bullet. Do not bold a word in the first paragraph.  Do not include non-human readable text such as XML ids.{searchModelInstructions2}

...
public class BillInterpretationService {
	...
	public static final String statsPromptTemplate = """
			...
			Rating:
			Write a single integer, from -100 to 100, which represents a voting recommendation score for the legislation (recommending voting either for (positive) or against (negative) the legislation). This rating should directly translate to a grade for the bill, as per the following grading rubric (where r is the rating): A: r >= 60, B: 60 < r >= 40, C: 40 < r >= 20, D: 20 < r >= 0, F: r < 0. If the impact to society is negative, then so too should the rating score - a bill with a negative impact is an F rating bill. This rating metric is separate from the impact scores, in that a bill might be of high rating but it might have a low impact. A common example might be a congressional gold medal award given for a heroic act (high rating), but the overall impact to society is small. Respond with only the integer, no commentary or units.

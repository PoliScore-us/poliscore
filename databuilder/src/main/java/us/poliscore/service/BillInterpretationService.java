...
public class BillInterpretationService {
	...
	public static final String statsPromptTemplate = """
			...
			Rating:
			Write a single integer, from -100 to 100, which represents a recommendation score for the legislation. This rating should directly translate to either a recommendation for or against the bill (as per the grading rubric). If the impact to society is negative, then so too should the rating score - a bill with a negative impact is an F rating bill. This metric is separate from the impact scores, in that a bill might be of high rating but it might have a low impact. A common example might be a congressional gold medal award given for a heroic act (high rating), but the overall impact to society is small. This rating score is used to determine an overall grade for this bill, and grades are assigned as follows (where r is the rating): A: r >= 60, B: 60 < r >= 40, C: 40 < r >= 20, D: 20 < r >= 0, F: r < 0. Respond with only the integer, no commentary or units.
